...
This module performs live HTTP requests against a running server. It is guarded
so it won't execute during `pytest` collection.
"""
...
def main() -> None:
    ...
    for test in tests:
        ...
        try:
            ...
            if resp.status_code in [200, 201] and test['method'] == 'POST' and test['endpoint'] == '/api/personnel/basic':
                payload = resp.json()
                print(f"  Created: ID={payload.get('id')}, Name={payload.get('first_name')} {payload.get('last_name')}")
